...
function seleccion_masiva() {
  ...
  var sheetForm = id.getSheetByName("Ingreso Metodización"); //var sheetWF = id.getSheetByName("WF");
  ...
  var data = sscc.getRange(2, 1, sscc.getRange("A10").getDataRegion().getLastRow(), sscc.getLastColumn()).getValues(); //Lectura de la base de Lourdes
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (analista_asignado == "LOURDES DEL PILAR GUTIERREZ MANCHEGO" && cliente_priorizado == "SÍ") {
      ...
      sheetWF.getRange(fEncontrada, cDevuelveCheckList).setValue(fecha); //sheetWF.getRange(fEncontrada,cArchivoDevuelve).setValue("N/A")
